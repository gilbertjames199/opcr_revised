            ...
            <div class="bgc-white p-20 bd">
                <div class="table-responsive">
                    <div class="fw-bold text-muted">
                        <h5><Link>Fetched {{ data.length }} {{ data.length === 1 ? 'result' : 'results' }}</Link></h5>
                    </div>
                        ...
                        <tbody>
                            <!-- :class="actionTypeClass(dat.action_type)" -->
                             <template v-for="dat in data" :key="dat.id" >
                                <tr  style="font-weight: bold" @click="showTrackings">
                                    <td :class="actionTypeClass(dat.action_type)">
                                        {{ formatType(dat.action_type) }}
                                    </td>
                                    <td><Link :href="`/revision/view/project/paps/${dat.revision_plan_id}`"
                                            target="_blank"
                                        >{{ dat.revision_plan.project_title }}</Link></td>
